...
const formRules = {
  name: [
    ...
    { min: 1, max: 20, message: '标签名称长度在1-20个字符之间' },
  ],
}
...
const pageInit = async (addition: { pageNum: number; pageSize: number }) => {
  ...
  tagStore.getTagListAction({ ...addition }).then(
    (res) => {
      // 重置属性
...
const headerConfig = computed(() => ({
  ...
  content: `您确定要删除选中的 ${selectedRowKeys.value.length} 个标签吗？删除后无法恢复。`,
}))
...
const handleBatchDeleteConfirm = async () => {
  pageInfo.loading = true
  try {
    await tagStore.delTagAction((selectedRowKeys.value as string[]) ?? [])
    // 清空选中的行
    ...
    >
      <AForm ref="formRef" :model="formData" :rules="formRules" layout="vertical">
        <AFormItem label="标签名称" name="name">
          <AInput v-model:value="formData.name" placeholder="请输入标签名称" maxlength="20" />
        </AFormItem>

        <AFormItem label="标签颜色" name="color">
